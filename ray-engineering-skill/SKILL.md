...
Goal: create or update repository-specific operating rules for agents, including a short verified-facts section for measured environment facts and pitfalls that future agents must not rederive from memory.
...
Goal: prove completion with evidence, starting with the narrowest relevant checks and broadening only as risk warrants. Any constraint that can be checked statically or automatically must be implemented as a script, linter, type rule, test, or CI check; manual checklist review is reserved for judgment calls.
...
Goal: keep current state, tasks, decisions, affected documentation, known limitations, verified facts, and next priority accurate after meaningful work.

Route: procedure `workflows/maintenance.md`; artifacts `templates/PROJECT_STATE.md`, `templates/CHANGELOG.md`, `templates/ADR.md`, `templates/AGENTS.md`, `templates/COMPLETION_REPORT.md`.
...
- When the phase last changed, what was the previous phase, the new phase, the trigger, and the date?

Do not use it as a historical log. History belongs in Git, changelog, issues, or decision records.

## Change classification

Quick autonomy check: if doing the change correctly would require changing an approved specification, architecture document, ADR, security boundary, or project state outside the current task, escalate or record the decision before continuing. If the design already covers the branch and only code/tests/docs generated by the task change, proceed autonomously. When unsure, use the four-level classification below.
...
- override a verified fact in `AGENTS.md` from memory or general intuition without new measured evidence
